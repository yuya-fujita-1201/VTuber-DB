...
function Admin() {
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      // 未承認タグ
      const tagsRes = await fetch(`${API_BASE_URL}/api/admin/unverified-tags`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
      if (tagsRes.ok) {
  ...
  const runSync = async (type) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/sync/${type}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const runAITagging = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/ai-tagging`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const verifyTag = async (vtuberId, tagId) => {
    const token = localStorage.getItem('admin_token');
    try {
      const res = await fetch(`${API_BASE_URL}/api/tags/verify`, {
        method: 'PUT',
        headers: {
  ...
  const removeTag = async (vtuberId, tagId) => {
    const token = localStorage.getItem('admin_token');
    try {
      const res = await fetch(`${API_BASE_URL}/api/tags/assign`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              ...
              {importResult && (
                <div className={`p-4 rounded-md ${importResult.success ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'
                  }`}>
